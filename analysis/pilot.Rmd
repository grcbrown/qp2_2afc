...
```{r load data}
## read messy dataset
df.pilot <- read_csv("./data/pilot2a.csv") # recode as df.responses
...
```{r}
df.pilot %>% 
  filter(label == "expressive" | label == "unexpressive") %>% # select responses of interest
  ggplot(mapping = aes(x = talker, 
                       y = response_ref,
                       fill = talker)) +
  stat_summary(fun = "mean",
...
```{r}
df.pilot %>% 
  filter(label == "smart" | label == "average") %>% # select responses of interest
  ggplot(mapping = aes(x = talker, 
                       y = response_ref,
                       fill = talker)) +
  stat_summary(fun = "mean",
...
```{r}
df.pilot %>% 
  filter(label == "cold" | label == "warm") %>% # select responses of interest
  ggplot(mapping = aes(x = talker, 
                       y = response_ref,
                       fill = talker)) +
  stat_summary(fun = "mean",
...
```{r}
df.pilot %>% 
  filter(label == "conventional" | label == "unconventional") %>% # select responses of interest
  ggplot(mapping = aes(x = talker, 
                       y = response_ref,
                       fill = talker)) +
  stat_summary(fun = "mean",
  ...
  coord_cartesian(ylim = c(0,1)) +
  labs(x = "Talker",
       y = "Mean Proportion 'Unconventional'") + 
  scale_fill_manual(values = cbPalette,
                    name = "Talker")
```
...
```{r}
df.pilot %>% 
  filter(label == "confident" | label == "insecure") %>% # select responses of interest
  ggplot(mapping = aes(x = talker, 
                       y = 1 - response_ref,
                       fill = talker)) +
  stat_summary(fun = "mean",
...
fit.sharp <- glmer(formula = response_ref ~ 1 + talker + 
                       (1 + talker | subject_id) +
                       (1 | sentence_id),
                 family = "binomial",
                   data = df.sharp)
